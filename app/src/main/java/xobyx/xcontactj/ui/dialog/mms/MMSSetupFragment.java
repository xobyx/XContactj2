package xobyx.xcontactj.ui.dialog.mms;
...
import xobyx.xcontactj.mmssms.Apn;
import xobyx.xcontactj.mmssms.ApnUtils;
...
public class MMSSetupFragment extends QKFragment implements MMSDialogFragment.DialogFragmentListener {
    ...
    public void onDialogFragmentResult(final int resultCode, DialogFragment fragment) {
        ...
        if (resultCode == MMSDialogFragment.DISMISS_RESULT) {
            close();
        }
        ...
        if (SET_UP_MMS.equals(dialogTag)) {
            if (resultCode == MMSDialogFragment.POSITIVE_BUTTON_RESULT) {
                // Show the dialog corresponding to the number of configurations that were found.
                showConfigurationDialog();

            } else if (resultCode == MMSDialogFragment.NEGATIVE_BUTTON_RESULT) {
            ...
            } else if (resultCode == MMSDialogFragment.NEUTRAL_BUTTON_RESULT) {
        ...
        // Multiple configurations were found and the user said that none of them looked right.
            ...
            if (resultCode == MMSDialogFragment.NEGATIVE_BUTTON_RESULT) {
    ...
    public void onDialogFragmentListResult(int resultCode, DialogFragment fragment, int index) {
        ...
        if (MULTIPLE_CONFIGURATIONS_FOUND.equals(dialogTag)) {
            if (resultCode == MMSDialogFragment.LIST_ITEM_CLICK_RESULT) {
